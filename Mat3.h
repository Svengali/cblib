...
public:
	...
	explicit __forceinline Mat3(const EConstructorRows,const Vec3 & rx,const Vec3 & ry,const Vec3 & rz)
		: m_x(rx) , m_y(ry) , m_z(rz)
	{
	}

	explicit __forceinline Mat3(const EConstructorCols,const Vec3 & cx,const Vec3 & cy,const Vec3 & cz)
	...
	explicit __forceinline Mat3(const EConstructorIdentity) 
		: m_x(1.f,0.f,0.f) , m_y(0.f,1.f,0.f) , m_z(0.f,0.f,1.f)
	{
	}
	
	explicit __forceinline Mat3(const EConstructorZero) 
...
//	void IO(const StreamPtr& str);
	...
	//! fuzzy equality test
	static bool Equals(const Mat3 &a,const Mat3 &b,const float tolerance = EPSILON);
	...
	I end up with Vec3::operator * function calls all over my
		optimized code!
	I must either change all my primitive types to not use eachother,
		or hope that VC7 is better !!!
	...
	*/
	...
	// IsRightHanded means det > 0
	bool IsRightHanded() const;
	
	//! IsOrthonormal means "IsRotation"
	//	(also checks right-handed)
	...
	bool IsUniformScale(const float tolerance = EPSILON) const;
...
// IsRightHanded means det > 0
inline bool Mat3::IsRightHanded() const
{
